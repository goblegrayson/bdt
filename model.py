...
class BlackDermanToy:
    ...
    def build_trees(self, r_vec, rate_tree, dt_vec):
        ...
        for i_step in range(i_solve, -1, -1):
            ...
            for i_node in range(i_step + 1):
                ...
                # Yield
                time_to_mat = np.sum(dt_vec[i_step:i_solve + 1])
                yield_tree[i_node, i_step] = self.calc_yield(self.par, price_tree[i_node, i_step], time_to_mat)
        # Output the resulting trees
        return rate_tree, price_tree, yield_tree

    def eval_trees(self, r_vec, rate_tree, dt_vec, yld_target=0, vol_target=0):
        rate_tree, price_tree, yield_tree = self.build_trees(r_vec, rate_tree, dt_vec)
        yld0 = yield_tree[0, 0]
    ...
    def update(self):
        ...
        dt_vec = np.diff(term_struct['mat'])
        dt_vec = np.append(dt_vec, 1)
        ...
        for i_step in range(1, n_steps):
            ...
            # Solve the step
            ru, rd = optimize.fsolve(self.eval_trees, x0, args=(rate_tree, dt_vec, yld_target, vol_target))
            if i_step == n_steps or self.debug:
                rate_tree, price_tree, yield_tree = self.build_trees((ru, rd), rate_tree, dt_vec)
            else:
                rate_tree, _, _ = self.build_trees((ru, rd), rate_tree, dt_vec)
        # Save the rate tree
